...
ifeq ($(DATA_IFACE), data)
	...
	ENB_SUBNET := $(shell ip route | grep $${DATA_IFACE} | awk '/proto/ {print $$1}')
endif
...
$(M)/omec: | node-prep
	...
	DATA_IFACE=${DATA_IFACE} ENB_SUBNET=${ENB_SUBNET} envsubst < $(4G_CORE_VALUES) | \
...
$(M)/5g-core: | node-prep
	...
	DATA_IFACE=${DATA_IFACE} ENB_SUBNET=${ENB_SUBNET} envsubst < $(5G_CORE_VALUES) | \
	helm upgrade --create-namespace --install --wait $(HELM_GLOBAL_ARGS) \
		--namespace omec \
		--values - \
		sd-core \
