...
5g-upf:
	...
	NODE_IP=${NODE_IP} DATA_IFACE=${DATA_IFACE} RAN_SUBNET=${RAN_SUBNET} envsubst < $(5G_UPF_VALUES) | \
	helm upgrade --create-namespace --install --wait $(HELM_GLOBAL_ARGS) \
		--namespace upf-${UPF_NUMBER} \
		--values - \
		bess-upf \
		$(UPF_CHART)
	$(eval IP_BEFORE=$(shell echo $$((3+$(UPF_NUMBER)-1))))
	$(eval IP_AFTER=$(shell echo $$((1+$(IP_BEFORE)))))
	$(eval IP_UE_BEFORE=$(shell echo $$((250+1-$(UPF_NUMBER)))))
	$(eval IP_UE_AFTER=$(shell echo $$(($(IP_UE_BEFORE)-1))))
	@sed -i 's/ip: 192.168.252.$(IP_BEFORE)/ip: 192.168.252.$(IP_AFTER)/' $(5G_UPF_VALUES)
	@sed -i 's/ip: 192.168.250.$(IP_BEFORE)/ip: 192.168.250.$(IP_AFTER)/' $(5G_UPF_VALUES)
	@sed -i 's/ue_ip_pool: "172.$(IP_UE_BEFORE).0.0/ue_ip_pool: "172.$(IP_UE_AFTER).0.0/' $(5G_UPF_VALUES)
	kubectl -n default exec -ti router -- ip route add 172.$(IP_UE_AFTER).0.0/16 via 192.168.250.$(IP_AFTER)
...
upf-clean:
	...
	$(eval IP_BEFORE=$(shell echo $$((3+$(UPF_NUMBER)-1))))
	$(eval IP_UE_BEFORE=$(shell echo $$((250+1-$(UPF_NUMBER)))))
	@sed -i 's/ip: 192.168.252.$(IP_BEFORE)/ip: 192.168.252.4/' $(5G_UPF_VALUES)
	@sed -i 's/ip: 192.168.250.$(IP_BEFORE)/ip: 192.168.250.4/' $(5G_UPF_VALUES)
	@sed -i 's/ue_ip_pool: "172.$(IP_UE_BEFORE).0.0/ue_ip_pool: "172.249.0.0/' $(5G_UPF_VALUES)
	@echo "1" > ${UPF_COUNT}
