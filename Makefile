...
roc-4g-models: $(M)/roc
	...
	if [ "${ENABLE_SUBSCRIBER_PROXY}" == "true" ] ; then sed -i 's/config4g/subscriber-proxy.aether-roc.svc.cluster.local/' $(4G_CORE_VALUES) ; fi	
...
roc-5g-models: $(M)/roc
	...
	if [ "${ENABLE_SUBSCRIBER_PROXY}" == "true" ] ; then sed -i 's/webui/subscriber-proxy.aether-roc.svc.cluster.local/' $(5G_CORE_VALUES) ; fi	
	$(eval ONOS_CLI_POD := $(shell kubectl -n aether-roc get pods -l name=onos-cli -o name))
	echo "ONOS CLI pod: ${ONOS_CLI_POD}"
	until kubectl -n aether-roc exec ${ONOS_CLI_POD} -- \
		curl -s -f -L -X PATCH "http://aether-roc-api:8181/aether-roc-api" \
