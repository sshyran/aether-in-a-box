...
node-prep: | $(M)/helm-ready /opt/cni/bin/static $(if $(ENABLE_ROUTER), $(M)/router) $(if $(ENABLE_OAISIM), $(M)/oaisim-node-prep)

4g-core: | $(M)/omec
$(M)/omec: | node-prep
	helm repo update
	if [[ "${CHARTS}" == "local" || "${CHARTS}" == "local-sdcore" ]]; then helm dep up $(SD_CORE_CHART); fi
	DATA_IFACE=$(DATA_IFACE) envsubst < $(4G_CORE_VALUES) | \
	helm upgrade --create-namespace --install --wait $(HELM_GLOBAL_ARGS) \
		--namespace omec \
		--set omec-user-plane.config.upf.access.iface=$(DATA_IFACE) \
		--set omec-user-plane.config.upf.core.iface=$(DATA_IFACE) \
		--values - \
		sd-core \
		$(SD_CORE_CHART)
	touch $@
...
$(M)/oaisim-node-prep: | $(M)/router
	sudo ip addr add 127.0.0.2/8 dev lo || true
	sudo ip link add enb link $(DATA_IFACE) type macvlan mode bridge
...
router-clean:
	kubectl delete net-attach-def router-net || true
	kubectl delete po router || true
	kubectl wait --for=delete -l app=router pod --timeout=180s || true
...
reset-test: omec-clean router-clean
	cd $(M); rm -f omec oaisim
...
clean: | $(if $(ENABLE_OAISIM), oaisim-clean) $(if $(ENABLE_ROUTER), router-clean)
	source "$(VENV)/bin/activate" && cd $(BUILD)/kubespray; \
