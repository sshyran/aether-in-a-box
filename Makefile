...
download-ue-image: | $(M)/k8s-ready-rke2
	sudo /var/lib/rancher/rke2/bin/ctr --address /run/k3s/containerd/containerd.sock --namespace k8s.io image pull docker.io/${OAISIM_UE_IMAGE} && \
		sudo /var/lib/rancher/rke2/bin/ctr --address /run/k3s/containerd/containerd.sock --namespace k8s.io image tag docker.io/${OAISIM_UE_IMAGE} \
			docker.io/omecproject/lte-uesoftmodem:1.1.0
	touch $(M)/ue-image /tmp/lte-uesoftmodem.tar

/tmp/lte-uesoftmodem.tar: $(BUILD)/openairinterface
	dpkg -l docker.io|grep '^ii' || sudo apt install -y docker.io
	sudo adduser $(USER) docker
