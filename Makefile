...
node-prep-rke2:	| $(M)/helm-ready-rke2 /opt/cni/bin/static
...
download-ue-image: | $(M)/k8s-ready-rke2
	/var/lib/rancher/rke2/bin/ctr --address /run/k3s/containerd/containerd.sock --namespace k8s.io image pull docker.io/${OAISIM_UE_IMAGE} && \
		/var/lib/rancher/rke2/bin/ctr --address /run/k3s/containerd/containerd.sock --namespace k8s.io image tag docker.io/${OAISIM_UE_IMAGE} \
			docker.io/omecproject/lte-uesoftmodem:1.1.0
	touch $(M)/ue-image

/tmp/lte-uesoftmodem.tar: $(BUILD)/openairinterface
	...
	sg docker -c "docker save -o /tmp/lte-uesoftmodem.tar omecproject/lte-uesoftmodem:1.1.0"

$(M)/ue-image: | $(M)/k8s-ready-rke2 /tmp/lte-uesoftmodem.tar
	/var/lib/rancher/rke2/bin/ctr --address /run/k3s/containerd/containerd.sock --namespace k8s.io images import /tmp/lte-uesoftmodem.tar
	touch $@
