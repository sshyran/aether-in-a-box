...
download-ue-image: | $(M)/k8s-ready-rke2
	ctr --address /run/k3s/containerd/containerd.sock --namespace k8s.io image pull docker.io/${OAISIM_UE_IMAGE} && \
		ctr --address /run/k3s/containerd/containerd.sock --namespace k8s.io image tag docker.io/${OAISIM_UE_IMAGE} \
...
$(M)/ue-image: | $(M)/k8s-ready-rke2 /tmp/lte-uesoftmodem.tar
	ctr --address /run/k3s/containerd/containerd.sock --namespace k8s.io images import /tmp/lte-uesoftmodem.tar
	touch $@
